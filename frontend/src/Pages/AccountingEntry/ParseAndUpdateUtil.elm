    ...
    let
        ...
        modelWithUpdatedContent =
            modelWithUpdatedEntry.content
               |> (\c -> InputContent.updateDay c (String.fromInt day))
...
updateMonth model month =
    let
        ...
        modelWithUpdatedContent =
            modelWithUpdatedEntry.content
               |> (\c -> InputContent.updateMonth c (String.fromInt month))
               |> updateContent modelWithUpdatedEntry
    in
      modelWithUpdatedContent
...
parseAndUpdateAmount model newContent =
    if String.isEmpty newContent then
        { model | content = updateAmount model.content ""
                                        ...
                                        case String.toInt (String.left 2 changeString) of
                                            Just change ->
                                                if change < 10 && String.length changeString == 1 then
                                                    { model | content = updateAmount model.content (String.concat [ String.fromInt whole, ",", String.fromInt change ])
                                                    ...
                                                    { model | content = updateAmount model.content  (String.concat [ String.fromInt whole, ",0", String.fromInt change ])
                                            ...
                                            Nothing ->
                                                { model | content = updateAmount model.content (String.concat [ String.fromInt whole, "," ])
...
parseMonth model newMonth  =
      case String.toInt newMonth of
          Just monthCandidate ->
                if  validDate monthCandidate model.accountingEntry.bookingDate.day model.accountingYear then
...
validDate dayCandidate monthCandidate year =
    ...
    List.member monthCandidate [0,1,3,5,7,8,10,12] && dayCandidate >=1 && dayCandidate <= 31
    || List.member monthCandidate [4,6,9,11] && dayCandidate >=1 && dayCandidate <= 30
